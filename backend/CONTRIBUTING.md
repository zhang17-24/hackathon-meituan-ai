# Contributing to nailflow Backend

Thank you for your interest in contributing to nailflow! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/nail-flow.git
   cd nail-flow
   ```
...
```python
import pytest
from nailflow.models.factory import create_chat_model
...
1. Create tool in `packages/harness/nailflow/tools/builtins/` or `packages/harness/nailflow/community/`:

```python
# packages/harness/nailflow/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: nailflow.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/nailflow/agents/middlewares/`:

```python
# packages/harness/nailflow/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/nailflow/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/nailflow/config/app_config.py` with new fields
...
Thank you for contributing to nailflow!
