...
from nailflow.agents.lead_agent.prompt import apply_prompt_template
from nailflow.agents.memory.summarization_hook import memory_flush_hook
from nailflow.agents.middlewares.clarification_middleware import ClarificationMiddleware
from nailflow.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
from nailflow.agents.middlewares.memory_middleware import MemoryMiddleware
from nailflow.agents.middlewares.safety_finish_reason_middleware import SafetyFinishReasonMiddleware
from nailflow.agents.middlewares.subagent_limit_middleware import SubagentLimitMiddleware
from nailflow.agents.middlewares.summarization_middleware import BeforeSummarizationHook, nailflowSummarizationMiddleware
from nailflow.agents.middlewares.title_middleware import TitleMiddleware
from nailflow.agents.middlewares.todo_middleware import TodoMiddleware
from nailflow.agents.middlewares.token_usage_middleware import TokenUsageMiddleware
from nailflow.agents.middlewares.tool_error_handling_middleware import build_lead_runtime_middlewares
from nailflow.agents.middlewares.view_image_middleware import ViewImageMiddleware
from nailflow.agents.thread_state import ThreadState
from nailflow.config.agents_config import load_agent_config, validate_agent_name
from nailflow.config.app_config import AppConfig, get_app_config
from nailflow.models import create_chat_model
from nailflow.skills.tool_policy import filter_tools_by_skill_allowed_tools
from nailflow.skills.types import Skill
from nailflow.tracing import build_tracing_callbacks
...
def _create_summarization_middleware(*, app_config: AppConfig | None = None) -> nailflowSummarizationMiddleware | None:
    ...
    # the sole entry point for nailflowSummarizationMiddleware, and the runtime
    # config is not expected to change after startup.
    skills_container_path = resolved_app_config.skills.container_path or "/mnt/skills"

    return nailflowSummarizationMiddleware(
        **kwargs,
...
def _create_todo_list_middleware(is_plan_mode: bool) -> TodoMiddleware | None:
    ...
    # Custom prompts matching nailflow's style
    system_prompt = """
<todo_list_system>
...
def _build_middlewares(
    ...
    *,
    ...
    from nailflow.agents.middlewares.dynamic_context_middleware import DynamicContextMiddleware
    ...
    if resolved_app_config.tool_search.enabled:
        from nailflow.agents.middlewares.deferred_tool_filter_middleware import DeferredToolFilterMiddleware
...
def _load_enabled_skills_for_tool_policy(available_skills: set[str] | None, *, app_config: AppConfig) -> list[Skill]:
    try:
        from nailflow.agents.lead_agent.prompt import get_enabled_skills_for_config
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    from nailflow.tools import get_available_tools
    from nailflow.tools.builtins import setup_agent, update_agent
