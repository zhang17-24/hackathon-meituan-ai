...
def _build_runtime_middlewares(
    *,
    ...
    from nailflow.agents.middlewares.llm_error_handling_middleware import LLMErrorHandlingMiddleware
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "nailflow"
    ...
    from nailflow.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
