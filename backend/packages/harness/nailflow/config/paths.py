...
from nailflow.config.runtime_paths import runtime_home
...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable nailflow state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.nail-flow``).  Using
...
class Paths:
    """
    Centralized path configuration for nailflow application data.
    ...
    BaseDir resolution (in priority order):
        ...
        3. Caller project fallback: `{project_root}/.nail-flow`
    """
