...
_DB_MODELS_CACHE: dict = {"data": None, "ts": 0.0}
_CACHE_TTL = 5.0


def _get_db_models(active_only: bool = True) -> list[dict]:
    """读取 nail_model_configs 中所有模型配置（带 5s 进程内缓存）。"""
    now = _time.monotonic()
    cached = _DB_MODELS_CACHE["data"]
    if cached is not None and (now - _DB_MODELS_CACHE["ts"]) < _CACHE_TTL:
        return cached
    try:
        ...
        _DB_MODELS_CACHE["data"] = result
        _DB_MODELS_CACHE["ts"] = now
        return result
...
def _model_matches_capability(model: dict, capability: Capability) -> bool:
    ...
    # CHAT: 跳过 vision-only 模型（识图专用模型不支持工具调用）
    if model.get("supports_vision") and not model.get("supports_thinking"):
        return False
    return True
