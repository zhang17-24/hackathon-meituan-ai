...
from nailflow.runtime.events.store.memory import MemoryRunEventStore
...
def make_run_event_store(config=None) -> RunEventStore:
    ...
    if config.backend == "db":
        ...
        if sf is None:
            ...
            return MemoryRunEventStore()
        from nailflow.runtime.events.store.db import DbRunEventStore

        return DbRunEventStore(sf, max_trace_content=config.max_trace_content)
    if config.backend == "jsonl":
        from nailflow.runtime.events.store.jsonl import JsonlRunEventStore
